...
struct EditExpenseSheet: View {
    ...
    init(expense: Expense) {
        ...
        _amountText         = State(initialValue: "\(Int(expense.amount))")
                    ...
                    VStack(spacing: 0) {
                        row("Title") {
                            TextField("", text: $title)
                                .font(.system(size: 15)).foregroundStyle(VeloceTheme.textPrimary)
                                .tint(VeloceTheme.accent)
                                .multilineTextAlignment(.trailing)
                        }
    ...
    private func row<C: View>(_ label: String, @ViewBuilder content: () -> C) -> some View {
        HStack {
            Text(label).font(.system(size: 14)).foregroundStyle(VeloceTheme.textSecondary)
            Spacer(minLength: 8)
            content()
        }
    }
